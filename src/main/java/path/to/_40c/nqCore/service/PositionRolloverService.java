...
public class PositionRolloverService {
    ...
    public void rollOver(String signalPrice) {
        ...
        if (tradeToRollOver != null) {
            ...
            Map<String, LTPQuote> ltpOfToCloseTrade = positionUtil.getLTP(liveIns);
            ...
            IntStream.range(0, tradeToRollOver.getLegs().size()).parallel().forEach(i -> {
                WeeklyLeg toClose = tradeToRollOver.getLegs().get(i);
                log.debug("WeeklyLeg to rollover is: {}", toClose);
                String oppositeTransaction = BUY.equals(toClose.getSide()) ? SELL : BUY;
                LTPQuote q = ltpOfToCloseTrade.get(toClose.getExchangeSymbol());
                if (q != null) {
                ...
                try {
                    ExecResult er = positionUtil.placeAggressiveOrder(toClose.getInstrument(), oppositeTransaction, toClose.getQuantity(), "EXIT");
            ...
            Map<String, LTPQuote> ltpOfToOpenTrade = positionUtil.getLTP(ltpIns);
            ...
            legOrder.forEach(pojo -> {
                ...
                LTPQuote q = ltpOfToOpenTrade.get(pojo.getExchangeSymbol());
                if (q != null) {
